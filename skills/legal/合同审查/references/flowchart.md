...
从合同文本提取完整交易流程，生成面向客户查看的业务流程图图片，并保留 Mermaid 源码作为可编辑中间文件。
...
- 必须同时输出 `.mmd` 源文件和 `.png` 图片；复杂图可同时输出 `.svg`
- Word、飞书文档和正式报告中嵌入图片，不直接嵌入 Mermaid 代码
- Mermaid 源码以 `flowchart TD` 或 `flowchart LR` 开头，语法必须正确可渲染
- 每张图后配2-3句说明，解释交易主线、付款/交付/验收节点和关键风险分支
- 单张图超过25个节点、文字拥挤或分支过多时，必须拆成“履行主线图”“付款节点图”“违约解除图”等多张图片
...
如未安装 `mmdc`，应改用其他可用渲染方式；无法生成图片时，不得称为正式交付完成，必须说明阻塞原因。
