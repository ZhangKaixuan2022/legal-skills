 ...
 * 广告合规审核报告 DOCX 生成器
 *
 * 历史非正式脚本：正式本地 Word 交付不得调用本脚本。
 * 正式交付必须走：draft.html -> 法律文书出稿前审查 -> 法律文书模板与导出。
 * 如确需做旧链路实验，需显式设置 LEGAL_ALLOW_LEGACY_DOCX=1。
 *
...
if (process.env.LEGAL_ALLOW_LEGACY_DOCX !== "1") {
  console.error("本脚本为历史非正式 DOCX 生成器，正式交付已禁用。请走 draft.html -> 法律文书出稿前审查 -> 法律文书模板与导出。");
  process.exit(2);
}
