...
- 已读取并记录该文档类型对应模板或格式标准，生成前说明关键格式要求，生成后按模板做反向结构检查；例如 `证据目录` 必须优先适用 `诉讼文书起草/templates/证据目录格式.md` 的分组文本段落形式，未经用户明确覆盖不得改成表格版。
...
- 原始文件作为只读来源已保留，修订稿另存为新 Word 文档，不覆盖、不改写原件。
