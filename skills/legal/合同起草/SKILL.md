---
name: 合同起草
description: 合同起草、生成合同、拟定协议、写合同、根据交易安排形成合同文本、多轮修订和最终版归档的唯一入口。基于三观四步法与本地合同库（22大类1000+模板），支持有模板时定制化起草、无模板时独立起草；输出合同草案，并同步生成客户可读合同编号、合同概览、业务摘要、版本记录、修订历史和续约提醒信息。
---
...
使用 `list_dir` 扫描合同库对应分类目录，判断模板匹配情况：
